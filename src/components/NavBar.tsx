...
const NavBar = () => {
    ...
    const unitSwitch = (isUnitChecked: boolean) => {
        return <div className='nav-option flex align-center justify-center'>
            F
                     <Switch
                color="default"
    ...
    const navLinks = (darkTheme: string) => {
        return <div className="flex nav-link-container justify-center">
            ...
            <NavLink activeClassName={"active"} className={`nav-option ${darkTheme}`} to="/Favorites" exact>
                Favorites
                   </NavLink>
        </div>
    }
    ...
    ) : <div onClick={() => setDrawer(true)} className='nav-container flex flex-end'>☰</div>
};
